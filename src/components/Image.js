...
export default function Image({ src, sizes, unoptimized = false, priority = false, loading, layout, className, quality, width, height, unsized, ...rest }) {
	if (process.env.NODE_ENV !== 'production') {
		if (!src) {
			throw new Error(`Image is missing required "src" property. Make sure you pass "src" in props to the \`next/image\` component. Received: ${JSON.stringify({ width, height, quality, unsized })}`)
		}
	...
	// No need to add preloads on the client side--by the time the application is hydrated, it's too late for preloads
	return (
