...
const Img = ({ alt, title, ...props }) => (
	<span className="block relative border-2 border-brand-400 shadow-brand mx-auto mb-4 overflow-hidden rounded-lg">
		<Image className="w-full" alt={alt} {...props} unsized loading="lazy" />
		{alt && <figcaption className={`italic px-4 py-1 text-left w-auto absolute font-medium mt-0 font-ttnorms border border-brand-400 bg-brand-100 bottom-0 rounded ${title == 'left' ? 'left-4' : 'right-4'} shadow-brand mb-4`}>{alt}</figcaption>}
	</span>
)
...
const Page = ({ title, description, image, showDate = true, children, publishedAt }) => {
	...
	return (
		<Layout title={title} description={description} image={image ?? `https://images.sitesauce.app/**${title}**.png?md=1&theme=brand_dark`}>
			<div className="bg-brand-50 overflow-hidden pt-16 z-10 relative">
